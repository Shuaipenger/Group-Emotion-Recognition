        ...
        for phase in range(0, 3):
            ...
            for i_batch, sample_batched in enumerate(dataloaders):
                ...
                with torch.set_grad_enabled(phase == 0):
                    output5 = model(image2)

                    if phase == 0:
                        output_train_model3.extend(output5.data.cpu().numpy())
                        label_train.extend(labels.cpu().numpy())
                    elif phase == 1:
                        output_valid_model3.extend(output5.data.cpu().numpy())
                        label_valid.extend(labels.cpu().numpy())
                    else:
                        output_test_model3.extend(output5.data.cpu().numpy())
                        label_test.extend(labels.cpu().numpy())

                    _, preds = torch.max(output5, 1)
                    loss = criterion(output5, labels)
