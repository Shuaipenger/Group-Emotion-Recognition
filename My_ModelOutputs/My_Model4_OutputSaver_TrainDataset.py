...
train_dataset = EmotiWDataset(train_filelist, root_dir, loadTrain=True, transformGlobal=train_global_data_transform,
                              transformFaces=train_faces_data_transform)
...
val_dataset = EmotiWDataset(val_filelist, root_dir, loadTrain=False, transformGlobal=val_global_data_transform,
                            transformFaces=val_faces_data_transform)

val_dataloader = DataLoader(val_dataset, shuffle=False, batch_size=batch_size, num_workers=0)

test_dataset = EmotiWDataset(test_filelist, root_dir, loadTrain=False, transformGlobal=test_global_data_transform,
                             transformFaces=test_faces_data_transform)
...
model4 = torch.load('../TrainedModels/TrainDataset_my_models/EfficientNet_skeletons', map_location=lambda storage, loc: storage)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in range(0, 3):
            ...
            for i_batch, sample_batched in enumerate(dataloaders):
                image2 = sample_batched['skeleton_image']
                ...
                with torch.set_grad_enabled(phase == 0):
                    ...
                    if phase == 0:
                        output_train_model4.extend(output.data.cpu().numpy())
                        label_train.extend(labels.cpu().numpy())
                    elif phase == 1:
                        output_valid_model4.extend(output.data.cpu().numpy())
                        label_valid.extend(labels.cpu().numpy())
                    else:
                        output_test_model4.extend(output.data.cpu().numpy())
                        label_test.extend(labels.cpu().numpy())

                    _, preds = torch.max(output, 1)
                    loss = criterion(output, labels)

                running_loss += loss.item() * image2.size(0)
                running_corrects += torch.sum(preds == labels.data)

            epoch_loss = running_loss / dataset_sizes[phase]
            epoch_acc = running_corrects.double() / dataset_sizes[phase]

            np.savez('model4_1_output_data',
