...
test_noface_data_transform = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
])
...
class wzq_Dataset(Dataset):
  ...
  # 没有检测到人脸
        ...
        features2 = np.zeros((16, 3, 224 , 224), dtype='float32')
        ...
        sample = {'image': image, 'features': features2, 'label': labeldict[labelname], 'numberFaces': numberCropsampling}
        return sample

train_dataset = wzq_Dataset(train_filelist, root_dir, loadTrain=True, transformGlobal=train_global_data_transform,transformFaces=train_noface_data_transform,loadTest = False)
train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=batch_size, num_workers=0)

val_dataset = wzq_Dataset(val_filelist, root_dir, loadTrain=False, transformGlobal=val_global_data_transform, transformFaces=val_noface_data_transform ,loadTest = False)
val_dataloader = DataLoader(val_dataset, shuffle=True, batch_size=batch_size, num_workers=0)

test_dataset = wzq_Dataset(test_filelist, root_dir, loadTrain=False, transformGlobal=test_global_data_transform, transformFaces=test_noface_data_transform ,loadTest = True)
...
class FaceAttention(nn.Module):
    ...
    def forward(self, image, face_features_initial, numberFaces, labels):
        # image: 32 3 224 224 batchsize  dim 图片大小     face_features_initial:32 4 3 224 224     numberFaces:32
        ...
        for j in range(batch_size):
            ...
            face = face_features_initial[j]  # face:16 3 224 224
            noface = self.global_model.forward(face)  # labels: 32
            out2 = F.relu(noface, inplace=False)  # out: 16 2208 7 7
            ...
            noface_features_initial = noface_features_initial.view(-1, 2208)  # global_features_initial: 16 2208
        ...
        face_features = self.align_model_dropout(face_features)     # 32 16 256
        # ———————————————————————————————————————————————————————————————————————————————————————————————————————————
...
model6_2 = torch.load('../TrainedModels/TrainDataset_my_models_wzq/model_6_2B_All_New_data', map_location=lambda storage, loc: storage)
print('Hybird Network model_NoFace Loaded! (Model 6_2)')

model_ft = model6_2
model_ft = model_ft.to(device)
model_ft = nn.DataParallel(model_ft)

output_train_model6_2 = []
output_valid_model6_2 = []
output_test_model6_2 = []
        ...
        for phase in range(0, 3):
            ...
            for i_batch, sample_batched in enumerate(dataloaders):
                ...
                with torch.set_grad_enabled(phase == 0):
                    ...
                    if phase == 0:
                        output_train_model6_2.extend(output1.data.cpu().numpy())
                        label_train.extend(labels.cpu().numpy())
                    elif phase == 1:
                        output_valid_model6_2.extend(output1.data.cpu().numpy())
                        label_valid.extend(labels.cpu().numpy())
                    else:
                        output_test_model6_2.extend(output1.data.cpu().numpy())
            ...
            np.savez('model6_2B_output_data10',
                     output_train_model6_2=output_train_model6_2,
                     output_valid_model6_2=output_valid_model6_2,
                     output_test_model6_2=output_test_model6_2,
