...
data_dir = '../Dataset/wzq_dataset7:2:1/AlignedCroppedImages/'
root_dir = "../Dataset/"
epochs = 1
batch_size = 8
maxFaces = 16
...
#用croppedfaces2文件夹中的目录，因为这里的都是检测到人脸的图片，排除了没有检测到人脸的图片
neg_train = sorted(os.listdir('/home/prmi/GSP/Group-Level-Emotion-Recognition-master/Dataset/wzq_dataset7:2:1/CroppedFaces2/train/'+'Negative/'))
pos_train = sorted(os.listdir('/home/prmi/GSP/Group-Level-Emotion-Recognition-master/Dataset/wzq_dataset7:2:1/CroppedFaces2/train/'+'Positive/'))

train_filelist = neg_train  + pos_train

neg_train = sorted(os.listdir('/home/prmi/GSP/Group-Level-Emotion-Recognition-master/Dataset/wzq_dataset7:2:1/CroppedFaces2/val/'+'Negative/'))
pos_train = sorted(os.listdir('/home/prmi/GSP/Group-Level-Emotion-Recognition-master/Dataset/wzq_dataset7:2:1/CroppedFaces2/val/'+'Positive/'))

val_filelist = neg_train  + pos_train

neg_train = sorted(os.listdir('/home/prmi/GSP/Group-Level-Emotion-Recognition-master/Dataset/wzq_dataset7:2:1/CroppedFaces2/test/'+'Negative/'))
pos_train = sorted(os.listdir('/home/prmi/GSP/Group-Level-Emotion-Recognition-master/Dataset/wzq_dataset7:2:1/CroppedFaces2/test/'+'Positive/'))

test_filelist = neg_train  + pos_train
...
train_dataset = wzq_Dataset(train_filelist, root_dir, loadTrain=True, transformGlobal=train_global_data_transform,transformFaces=train_faces_data_transform,loadTest = False)

train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=batch_size, num_workers=0)

val_dataset = wzq_Dataset(val_filelist, root_dir, loadTrain=False, transformGlobal=val_global_data_transform, transformFaces=val_faces_data_transform,loadTest = False)

val_dataloader = DataLoader(val_dataset, shuffle=True, batch_size=batch_size, num_workers=0)

test_dataset = wzq_Dataset(test_filelist, root_dir, loadTrain=False, transformGlobal=test_global_data_transform, transformFaces=test_faces_data_transform,loadTest = True)
...
model6_1 = torch.load('../TrainedModels/TrainDataset_my_models_wzq/model_6_1A_All_New_data', map_location=lambda storage, loc: storage)
print('Hybird Network model_Face Loaded! (Model 6_1)')

model_ft = model6_1
model_ft = model_ft.to(device)
model_ft = nn.DataParallel(model_ft)

output_train_model6_1 = []
output_valid_model6_1 = []
output_test_model6_1 = []
        ...
        for phase in range(0, 3):
            ...
            for i_batch, sample_batched in enumerate(dataloaders):
                ...
                with torch.set_grad_enabled(phase == 0):
                    output1 = model(inputs, face_features, numberFaces, labels)

                    if phase == 0:
                        output_train_model6_1.extend(output1.data.cpu().numpy())
                        label_train.extend(labels.cpu().numpy())
                    elif phase == 1:
                        output_valid_model6_1.extend(output1.data.cpu().numpy())
                        label_valid.extend(labels.cpu().numpy())
                    else:
                        output_test_model6_1.extend(output1.data.cpu().numpy())
            ...
            np.savez('model6_1_output_data',
                     output_train_model6_1=output_train_model6_1,
                     output_valid_model6_1=output_valid_model6_1,
                     output_test_model6_1=output_test_model6_1,
