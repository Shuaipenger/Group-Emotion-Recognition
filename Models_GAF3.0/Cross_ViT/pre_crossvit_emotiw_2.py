...
class CrossViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.lg_mlp_head_linear = nn.Linear(sm_dim, num_classes)
        ...
        for phase in range(2):
            ...
            for i_batch, sample_batched in enumerate(dataloaders):
                ...
                with torch.set_grad_enabled(phase == 0):   #查一下作用   设置梯度计算开启或关闭的上下文管理器  phase == 0时开启，反之关闭
                    outputs = model(face_features,inputs)  # 输入:face_features:32 16 3 96 112   inputs:32 3 256 256
...
torch.save(model_ft.state_dict(), "../TrainedModels/pre_crossvit_emotiw_2.pt")
torch.save(model_ft, '../TrainedModels/TrainDataset/pre_crossvit_emotiw_2.pt')