...
root_dir = "../Dataset/"
data_dir = '../Dataset/emotiw_skeletons/'
...
neg_train = sorted(os.listdir('../Dataset/emotiw_skeletons/train/'+'Negative/'))
neu_train = sorted(os.listdir('../Dataset/emotiw_skeletons/train/'+'Neutral/'))
pos_train = sorted(os.listdir('../Dataset/emotiw_skeletons/train/'+'Positive/'))
...
class EmotiWDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.loadTrain:
            train = 'train'
            image = Image.open(self.root_dir + 'emotiw_skeletons/' + train + '/' + labelname + '/' + filename + '.png')
        else:
            train = 'val'
            image = Image.open(self.root_dir + 'emotiw_skeletons/' + train + '/' + labelname + '/' + filename + '_rendered.png')
...
# model_ft = models.efficientnet_v2_m(pretrained=True)

# model_ft = models.inception_v3(pretrained=True)

model_ft = models.efficientnet_b6(pretrained=True)
model_ft.classifier = nn.Sequential( nn.Dropout(p=0.5,inplace=True), nn.Linear(in_features=2304,out_features=3,bias=True))
    ...
    for epoch in range(num_epochs):
        ...
        for phase in range(2):
            ...
            if phase == 1 and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model_ft, '../TrainedModels/TrainDataset_my_models/model_4_2_EfficientNet_skeletons_new')
