...
class wzq_Dataset(Dataset):
    ...
    def __init__(self, filelist, root_dir, loadTrain=True, transformGlobal=transforms.ToTensor(),transformFaces=transforms.ToTensor(),transformnoFace=transforms.ToTensor(), loadTest = False):
        """
        Args:
...
train_dataset = wzq_Dataset(train_filelist, root_dir, loadTrain=True, transformGlobal=train_global_data_transform,transformFaces=train_faces_data_transform,transformnoFace=train_noface_data_transform,loadTest = False)

train_dataloader = DataLoader(train_dataset, shuffle=False, batch_size=batch_size, num_workers=0)

val_dataset = wzq_Dataset(val_filelist, root_dir, loadTrain=False, transformGlobal=val_global_data_transform, transformFaces=val_faces_data_transform,transformnoFace=val_noface_data_transform,loadTest = False)

val_dataloader = DataLoader(val_dataset, shuffle=False, batch_size=batch_size, num_workers=0)

test_dataset = wzq_Dataset(test_filelist, root_dir, loadTrain=False, transformGlobal=test_global_data_transform, transformFaces=test_faces_data_transform,transformnoFace=test_noface_data_transform,loadTest = True)
...
model5 = torch.load('../TrainedModels/TrainDataset_my_models_wzq/model_5_2_All_New_data', map_location=lambda storage, loc: storage)
...
def train_model(model, criterion, optimizer=None, scheduler=None, num_epochs=25):
    since = time.time()
        ...
        for phase in range(0, 3):
            ...
            for i_batch, sample_batched in enumerate(dataloaders):
                ...
                with torch.set_grad_enabled(phase == 0):
                    ...
                    if phase == 0:
                        output_train_model5.extend(outputs.data.cpu().numpy())
                        label_train.extend(labels.cpu().numpy())
                    elif phase == 1:
                        output_valid_model5.extend(outputs.data.cpu().numpy())
                        label_valid.extend(labels.cpu().numpy())
                    else:
                        output_test_model5.extend(outputs.data.cpu().numpy())
            ...
            np.savez('model5_2_output_data_onecrossvit.npz',
