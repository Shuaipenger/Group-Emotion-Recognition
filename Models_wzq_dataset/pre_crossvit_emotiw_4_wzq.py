...
# 用的是wzq数据库里有人脸的图片进行的训练 pre_crossvit_wzq_1.pt用的是neg_train = sorted(os.listdir('../Dataset/wzq_dataset/FaceFeatures2/
#                                    pre_crossvit_wzq_2.pt用的是neg_train = sorted(os.listdir('../Dataset/wzq_dataset7:2:1/CroppedFaces2/
...
root_dir = "../Dataset/"
data_dir = '../Dataset/wzq_dataset7:2:1/'
batch_size = 32
epochs = 24
...
# DATASET AND LOADERS
#---------------------------------------------------------------------------
neg_train = sorted(os.listdir('../Dataset/wzq_dataset7:2:1/CroppedFaces2/train/'+'Negative/'))
pos_train = sorted(os.listdir('../Dataset/wzq_dataset7:2:1/CroppedFaces2/train/'+'Positive/'))

train_filelist = neg_train  + pos_train

neg_val = sorted(os.listdir('../Dataset/wzq_dataset7:2:1/CroppedFaces2/val/' + 'Negative/'))
pos_val = sorted(os.listdir('../Dataset/wzq_dataset7:2:1/CroppedFaces2/val/' + 'Positive/'))
...
class wzq_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features3 = np.load(self.root_dir + 'wzq_dataset7:2:1/GlobalFeatures/' + train + '/' + labelname + '/' + filename + '.npz')['a']

        sample = {'image': image, 'label': labeldict[labelname], 'numberFaces': numberFaces,'face_features': features2,'global_features':features3}
...
class CrossViT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.sm_mlp_head = nn.LayerNorm(sm_dim) #原来是 LayerNorm
    ...
    for epoch in range(num_epochs):
        ...
        for phase in range(2):
            ...
            if phase == 1 and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model, '../TrainedModels/TrainDataset_my_models_wzq/pre_crossvit_wzq_New_data')
            if phase ==0:
                scheduler.step()  # 改动
        print()
...
torch.save(model_ft.state_dict(), "../TrainedModels/TrainDataset_my_models_wzq/pre_crossvit_wzq_New_data.pt")
torch.save(model_ft, '../TrainedModels/TrainDataset_my_models_wzq/pre_crossvit_wzq_New_data')