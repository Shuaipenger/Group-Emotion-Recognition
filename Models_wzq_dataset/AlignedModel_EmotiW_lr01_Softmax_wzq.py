...
train_dataset = wzq_Dataset(train_filelist, root_dir, loadTrain = True, transformFaces = train_faces_data_transform)

train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=batch_size, num_workers=0)

val_dataset = wzq_Dataset(val_filelist, root_dir, loadTrain=False, transformFaces = val_faces_data_transform)
...
model_ft.fc6 = nn.Linear(512,2)   #将LSoftmaxLoss()换成 linear()
    ...
    for epoch in range(num_epochs):
        ...
        for phase in range(2):
            ...
            for i_batch, sample_batched in enumerate(dataloaders):
                ...
                face_features = sample_batched['features']   # batch maxface 3 96 112
                ...
                with torch.set_grad_enabled(phase == 0):
                    ...
                    labels = la.to(device) #  batch*maxface

                    for i in range(maxFaces): #   循环似乎没有用
            ...
            if phase == 1 and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model, '../TrainedModels/TrainDataset_my_models_wzq/AlignedModel_New_data')
            if phase == 0:
                scheduler.step()  # 改动
        print()
...
torch.save(model_ft, '../TrainedModels/TrainDataset_my_models_wzq/AlignedModel_New_data')
