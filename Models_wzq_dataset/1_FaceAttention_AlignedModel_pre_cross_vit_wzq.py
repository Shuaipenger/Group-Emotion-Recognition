...
aligned_path = '../TrainedModels/TrainDataset_my_models_wzq/AlignedModel_EmotiW_lr01_Softmax'
...
neg_train = sorted(os.listdir('../Dataset/wzq_dataset/train/' + 'Negative/'))
pos_train = sorted(os.listdir('../Dataset/wzq_dataset/train/' + 'Positive/'))
train_filelist = neg_train  + pos_train
neg_val = sorted(os.listdir('../Dataset/wzq_dataset/val/' + 'Negative/'))
pos_val = sorted(os.listdir('../Dataset/wzq_dataset/val/' + 'Positive/'))
...
class wzq_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(self.root_dir + 'wzq_dataset/' + train + '/' + labelname + '/' + filename + '.jpg')
        ...
        if  os.path.isfile(self.root_dir + 'wzq_dataset/AlignedCroppedImages/' + train + '/' + labelname + '/' + filename + '_0.jpg' ):
            features = np.load(self.root_dir + 'wzq_dataset/FaceFeatures2/' + train + '/' + labelname + '/' + filename + '.npz')['a']
            ...
            for i in range(maxNumber):
                face = Image.open(self.root_dir + 'wzq_dataset/AlignedCroppedImages/' + train + '/' + labelname + '/' + filename + '_' + str(i) + '.jpg')
            ...
            features1 = np.zeros((numberCropsampling, 3, 96, 112), dtype='float32')

            for i in range(numberCropsampling):
                face = Image.open(self.root_dir + 'wzq_dataset/FaceFeatures_Crop_sampling/' + train + '/' + labelname + '/' + filename + '_' + str(i) + '.jpg')
                if self.transformFaces:
                    face = self.transformFaces(face)
            ...
            features1 = torch.from_numpy(features1)
            sample = {'image': image, 'features': features1, 'label': labeldict[labelname], 'numberFaces': numberCropsampling}
        return sample

train_dataset = wzq_Dataset(train_filelist, root_dir, loadTrain=True, transformGlobal=train_global_data_transform,transformFaces=train_faces_data_transform)

train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=batch_size, num_workers=0)

val_dataset = wzq_Dataset(val_filelist, root_dir, loadTrain=False, transformGlobal=val_global_data_transform, transformFaces=val_faces_data_transform)
...
global_model.load_state_dict(torch.load('../TrainedModels/TrainDataset_my_models_wzq/model_1_1_densenet_emotiw_lr001.pt', map_location=lambda storage, loc: storage))
global_model = global_model.module.features
...
crossvit = torch.load('../TrainedModels/TrainDataset_my_models_wzq/pre_crossvit_wzq_1', map_location=lambda storage, loc: storage).module
    ...
    for epoch in range(num_epochs):
        ...
        for phase in range(2):
            ...
            if phase == 1 and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model,'../TrainedModels/TrainDataset_my_models_wzq/model_5_1_FaceAttention_AlignedModel_FullTrain_lr001_dropout_BN_SoftmaxLr01')
        scheduler.step()  # 改动
        print()
