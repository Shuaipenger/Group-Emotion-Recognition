...
neg_train = sorted(os.listdir('../Dataset/wzq_dataset7:2:1/val_skeleton/' + 'Negative/'))
pos_train = sorted(os.listdir('../Dataset/wzq_dataset7:2:1/val_skeleton/' + 'Positive/'))
...
model_ft.classifier = nn.Sequential( nn.Dropout(p=0.5,inplace=True), nn.Linear(in_features=2304,out_features=2,bias=True))
    ...
    for epoch in range(num_epochs):
        ...
        for phase in range(2):
            ...
            if phase == 1 and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model_ft, '../TrainedModels/TrainDataset_my_models_wzq/model_skeleton_EfficientNet_New_data.pt')
            if phase == 0:
                scheduler.step()  # 改动
        print()
