...
# IMPORTING MODULES
# ----------------------------------------------------------------------------
#模型用于方案一，无人脸时。其中固定了densenet161模型参数    用的新的CropFaces2作为划分有无人脸
...
train_dataset = wzq_Dataset(train_filelist, root_dir, loadTrain=True, transformGlobal=train_global_data_transform,transformFaces=train_noface_data_transform)
train_dataloader = DataLoader(train_dataset, shuffle=True, batch_size=batch_size, num_workers=0)

val_dataset = wzq_Dataset(val_filelist, root_dir, loadTrain=False, transformGlobal=val_global_data_transform, transformFaces=val_noface_data_transform)
...
for param in align_model.parameters():  ##后加的
    param.requires_grad = False  ##后加的
...
# for param in crossvit.parameters():  ##后加的
#     param.requires_grad = False  ##后加的
    ...
    for epoch in range(num_epochs):
        ...
        for phase in range(2):
            ...
            if phase == 1 and epoch_acc > best_acc:
                best_acc = epoch_acc
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model,'../TrainedModels/TrainDataset_my_models_wzq/model_6_2B_All_New_data3')
            if phase ==0:
                scheduler.step()  # 改动
        print()
...
optimizer_ft = optim.SGD(model_ft.parameters(), lr=0.01, momentum=0.9) #修改了lr ,原来为0.001
