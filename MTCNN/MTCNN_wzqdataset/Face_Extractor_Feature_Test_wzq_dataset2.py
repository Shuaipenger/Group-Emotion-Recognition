...
processed_dataset_path = '/home/prmi/GSP/Group-Level-Emotion-Recognition-master/Dataset/wzq_dataset/FaceFeatures2/'
        ...
        if label == 0:
            ...
            if (type(features)) == tuple:
                with open('hello.text', 'a') as f:
                    f.write(test_filelist[i])
                continue
            features = features.data.numpy()

            if features.size == 0:
                print('MTCNN model handling empty face condition at ' + test_filelist[i])
            np.savez(processed_dataset_path + 'test/Negative/' + test_filelist[i], a=features)
            ...
            if features.size == 0:
                print('MTCNN model handling empty face condition at ' + test_filelist[i])
            np.savez(processed_dataset_path + 'test/Positive/' + test_filelist[i], a=features)
